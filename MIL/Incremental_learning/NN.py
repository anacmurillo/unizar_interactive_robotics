...
class NN():
    ...
    def reorganize(self,point1,weight):
        ...
        for i in xrange(len(point1)):
            p = np.float32(point1[i][0])
            point1_sub = [(np.float32(pt[0]),p[1]) for pt in point1[0:i]+point1[i+1:len(point1)]]
            #weight_sub = weight[:]
            ...
            if self.weight_flag:
                ...
                #weight_sub.pop(i)
            #point1_sub.pop(i)
            distance = self.distance(point1_sub,None,p)
            d.append(distance)
            if distance < distance_global :
                distance_global=distance
                index = i
                value = self.aux
        point1 = [pt for pt in point1[0:index]+point1[index+1:len(point1)]]
    ...
    def distance_hist(self,point1,point2):
        return cv2.compareHist(point1[0],point2,cv2.HISTCMP_BHATTACHARYYA)

    def user_hist(self):
        user = ["user" + m.__str__() for m in xrange(1, 11)]
        N = len(self.labels)
        ind = np.arange(N)
        p = []
        width = 0.35
        ...
        for u in user:
            s = []
            for i in xrange(len(self.classes)):
                n = np.unique(np.array([tr[1] for tr in self.classes[i]]),return_counts=True)
                if u in n[0]:
                    s.append(n[1][n[0].tolist().index(u)])
                else:
                    s.append(0)
            s = np.array(s)
            if p != []:
                p.append(plt.bar(ind,s,width,bottom=last))
                last = last+s
            else:
                last = s
                p.append(plt.bar(ind,s,width))
        plt.ylabel('Data stored')
        plt.title('Data stored by user')
        plt.xticks(ind,self.labels,rotation=17)
        #plt.yticks(np.arange(0, 100, 1))
        plt.legend(p, user,loc='upper right')
        f.canvas.draw()
        data = np.fromstring(f.canvas.tostring_rgb(),dtype='uint8')
        image = data.reshape(f.canvas.get_width_height()[::-1] + (3,))
        plt.close(f)
        return image
    ...
    def test(self,des, Number):
        if len(self.classes) <= 0:
            ...
            aux = None
            for i in xrange(len(self.classes)):
                d = self.distance([(np.float32(p[0]),p[1]) for p in self.classes[i]],None,np.float32(des))
                if Number == 1:
    ...
    def ntest(self,des,n):
        if len(self.classes) <= 0:
            return -1,"",0
        else:
            out = []
            for i in xrange(len(self.classes)):
                for p in self.classes[i]:
                    d = self.distance_hist((np.float32(p[0]),p[1]),np.float32(des))
                    out.append((self.labels[i],d))
            out= sorted(out, key=lambda s: s[1])
            out = out[0:n]
